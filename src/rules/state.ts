...
/**
 * Край Карты. В раскладке odd-r строки идут с севера на юг, поэтому «север» —
 * это `row: 0`, «юг» — последняя строка; статья знает только эту пару, но
 * запад и восток выписаны, чтобы Карта могла поставить Обоз поперёк.
 */
export type BoardEdge = "north" | "south" | "east" | "west";
...
export type Board = {
  ...
  /**
   * Гексы, на которых сторона вправе выставить Отряды до начала Боя. Набор, а
   * не «край плюс глубина»: полоса выражается набором, а осадный сценарий
   * полосой — нет. Карта без этого поля не ограничивает расстановку ничем
   * (`validateSetup` промолчит про зоны — см. тикет 10).
   */
  readonly deployment?: Readonly<Record<Side, readonly Hex[]>>;
  /**
   * Край Карты, принадлежащий стороне: туда её дальнобойный Отряд уходит за
   * Боезапасом и через него же уходят её Отряды в Бегстве. Карта без этого
   * поля сохраняет прежнее поведение — годится любой край доски (тикет 09).
   * Статья Обоз не описывает, прочтение выведено (`docs/adr/0007`).
   */
  readonly baggage?: Readonly<Record<Side, BoardEdge>>;
};
